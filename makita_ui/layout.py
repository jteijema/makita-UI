...
def _template_layout(templates, data_dir):
    ...
    if os.path.exists(data_dir):
        ...
        dataFolderString = sg.Multiline(data_files, size=(60, 4), disabled=True, autoscroll=True)
    else:
    ...
    return [
        ...
        [sg.Text('Template'),           sg.Push(), sg.Combo(templates, size=38, key="-TEMPLATE-", enable_events=True)],
        [sg.Text('output_dir'),         sg.Push(), sg.InputText(key='template_output_dir', size=40, default_text='output')],
        [sg.Text('init_seed'),          sg.Push(), sg.InputText(key='init_seed', size=40, default_text='400')],
        [sg.Text('model_seed'),         sg.Push(), sg.InputText(key='model_seed', size=40, default_text='250')],
        [sg.Text('n_priors'),           sg.Push(), sg.InputText(key='n_priors', size=40, default_text='1', visible=False)],
        [sg.Text('n_runs'),             sg.Push(), sg.InputText(key='n_runs', size=40, default_text='1', visible=False)],
        [sg.Text('classifiers'),        sg.Push(), sg.InputText(key='classifiers', size=40, default_text='logistic nb rf svm', visible=False)],
        [sg.Text('feature_extractors'), sg.Push(), sg.InputText(key='feature_extractors', size=40, default_text='doc2vec sbert tfidf', visible=False)],
...
def _script_layout(available_scripts):
    ...
    return [
        ...
        [sg.VPush()],
        [sg.Button("Back", button_color=("white", sg.theme_background_color()))]
    ]
